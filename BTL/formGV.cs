...
namespace BTL
{
    public partial class formGV : Form
    {
        ...
        {
            DialogResult kq = MessageBox.Show("Bạn có muốn xóa không?", "Thông báo", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
            if (kq == DialogResult.Yes)
            {
                using (SqlConnection cnn = new SqlConnection(constr))
                {
                    string IDGV = tbMaGV.Text.Trim();

                    // Dùng cú pháp định dạng chuỗi để thay thế giá trị
                    string query = $@"
                UPDATE SinhVien
                SET maLopHC_FK = 'OLD{IDGV}'  
                WHERE maLopHC_FK IN (SELECT maLopHC FROM LopHanhChinh WHERE maGVChuNhiemFK = '{IDGV}');
    
                UPDATE LopHanhChinh
                SET maGVChuNhiemFK = 'OLD{IDGV}' 
                WHERE maGVChuNhiemFK = '{IDGV}'; 
    
                UPDATE LopTinChi
                SET maGiangVienFK = 'OLD{IDGV}' 
                WHERE maGiangVienFK = '{IDGV}';
